 ...
 */
function applyAllMigrationsSqlDirectly(sqlite: InstanceType<typeof Database>): void {
...
/**
 * __drizzle_migrationsテーブルを作成し、全マイグレーションを「適用済み」として記録する
 * Drizzleのmigrate()内部と同じテーブル構造・ハッシュ計算方式を使用する
 *
 * これにより、将来マイグレーションが追加された際も、
 * migrate()は既存のマイグレーションをスキップし、新しいもののみ適用する
 */
function stampAllMigrationsAsApplied(sqlite: InstanceType<typeof Database>): void {
  // Drizzleのmigrate()が作成するのと同じテーブル構造
  sqlite.exec(`
  ...
  for (const entry of journal.entries) {
    ...
    const migrationSql = fs.readFileSync(sqlFile, 'utf-8');
    // Drizzleと同じMD5ハッシュ計算
    const hash = crypto.createHash('md5').update(migrationSql).digest('hex');
...
console.log('\n=== シナリオ2: 既存DB(データ入り) -> migrate() -> データ保全確認 ===');
{
  ...
  // マイグレーションSQLを直接実行してテーブルを作成し、
  // __drizzle_migrationsに全マイグレーションを「適用済み」として記録する
  // これは「既にDrizzle migrate()を通して最新状態になったDB」を再現する
  try {
    applyAllMigrationsSqlDirectly(sqlite);
    stampAllMigrationsAsApplied(sqlite);
    assert(true, 'テーブル作成+マイグレーション記録成功');
  } catch (e) {
  ...
  assert(appliedCount === expectedMigrationEntries, `__drizzle_migrations: ${appliedCount}エントリ（適用済み）`);
  ...
  // migrate()後も__drizzle_migrationsのエントリ数が変化しないことを確認
  // （全マイグレーションが既に適用済みとして認識される）
  ...
  assert(migrationsAfter.length === expectedMigrationEntries,
    `__drizzle_migrations: ${migrationsAfter.length}エントリ（変化なし）`);
