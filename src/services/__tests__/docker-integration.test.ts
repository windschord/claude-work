...
const SANDBOX_IMAGE = dockerService.getFullImageName();
...
function dockerImageExists(): boolean {
  try {
    const result = execSync(`docker images -q ${SANDBOX_IMAGE}`, {
      encoding: 'utf-8',
    }).trim();
  ...
  describeWithImage('Docker Container Lifecycle', () => {
    ...
    it('should run container with volume mount', () => {
      ...
      const result = execSync(
        `docker run --rm -v "${tempDir}:/workspace:ro" ${SANDBOX_IMAGE} cat /workspace/test.txt`,
        { encoding: 'utf-8' }
      );
    ...
    it('should run container with correct user', () => {
      ...
      const result = execSync(
        `docker run --rm ${SANDBOX_IMAGE} whoami`,
        { encoding: 'utf-8' }
      );
    ...
    it('should have claude command available', () => {
      const result = execSync(
        `docker run --rm ${SANDBOX_IMAGE} which claude`,
        { encoding: 'utf-8' }
      );
    ...
    it('should respect security options', () => {
      ...
      expect(() => {
        execSync(
          `docker run --rm --cap-drop ALL ${SANDBOX_IMAGE} mount -t proc proc /proc 2>&1`,
          { encoding: 'utf-8' }
        );
      }).toThrow();
    });
    ...
    it('should be able to write to workspace', () => {
      // ワークスペースへの書き込みテスト
      const result = execSync(
        `docker run --rm -v "${tempDir}:/workspace" ${SANDBOX_IMAGE} sh -c "echo 'written by container' > /workspace/output.txt && cat /workspace/output.txt"`,
        { encoding: 'utf-8' }
      );
    ...
    it('should have git available', () => {
      const result = execSync(
        `docker run --rm ${SANDBOX_IMAGE} git --version`,
        { encoding: 'utf-8' }
      );
    ...
    it('should have node available', () => {
      const result = execSync(
        `docker run --rm ${SANDBOX_IMAGE} node --version`,
        { encoding: 'utf-8' }
      );
...
describe('Docker Test Environment Info', () => {
  it('should report Docker availability', () => {
    ...
    if (!DOCKER_AVAILABLE) {
      ...
      console.log(`Run: docker build -t ${SANDBOX_IMAGE} docker/`);
    }
