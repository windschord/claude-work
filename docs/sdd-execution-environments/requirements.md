...
- REQ-EE026: Dockerfileビルド選択時、システムはDockerfileのパスを入力できるフォームを提供しなければならない
- REQ-EE027: Dockerfileビルド選択時、環境作成時にシステムは指定されたDockerfileからイメージをビルドしなければならない
- REQ-EE028: 環境カードにおいて、システムは使用中のイメージ名（またはDockerfileパス）を表示しなければならない
