...
### ストーリー8: サイドメニューからのセッション作成改善

**私は** Claude Workのユーザーとして
**〜したい** サイドメニューからセッションを作成する際に実行環境を選択
**なぜなら** ワンクリックで適切な環境でセッションを開始したいから

#### 受入基準（EARS記法）

- REQ-EE032: サイドメニューの「新規セッション」クリック時、システムは実行環境選択モーダルを表示しなければならない
- REQ-EE033: モーダルにおいて、システムは利用可能な実行環境一覧をラジオボタンまたはドロップダウンで表示しなければならない
- REQ-EE034: モーダルでデフォルト環境が初期選択されていなければならない
- REQ-EE035: ユーザーが環境を選択して作成ボタンをクリックした時、システムはその環境でセッションを作成しなければならない
- REQ-EE036: プロジェクト詳細ページ（/projects/[id]）は削除されなければならない
- REQ-EE037: サイドメニューでプロジェクト名クリック時、システムはプロジェクトを展開/折りたたみしなければならない（ページ遷移なし）
...
| P1（重要） | REQ-EE032, REQ-EE033, REQ-EE034, REQ-EE035, REQ-EE036, REQ-EE037 | サイドメニュー改善 |
