...
- [x] タスク23.1が完了している（環境変数ロード修正）
- [x] タスク23.2が完了している（Prismaクライアント修正）
- [x] ログインページで正しいトークンを入力してログイン成功する
- [x] ログイン後、プロジェクト一覧ページが表示される
- [x] セッションCookieが正しく機能し、認証状態が維持される
- [x] `/api/auth/session`が認証済みユーザーに対して`{"authenticated": true}`を返す
- [x] すべてのコミットが作成されている
- [x] 動作検証チェックリスト（docs/verification-checklist.md）のログイン関連項目がパスする
...
**受入基準**:
- [x] `src/components/sessions/GitStatusBadge.tsx`ファイルが作成されている
- [x] GitStatusBadgeコンポーネントがprops `isDirty: boolean`を受け取る
- [x] isDirty=trueで「変更あり」バッジが表示される
- [x] isDirty=falseで「クリーン」バッジが表示される
- [x] Tailwind CSSでレスポンシブ対応されている
- [x] `src/components/sessions/__tests__/GitStatusBadge.test.tsx`のすべてのテストが通過する
- [x] SessionCard.tsxにGitStatusBadgeが統合されている
- [x] ESLintエラーがゼロである
- [x] コミットが存在する
...
**受入基準**:
- [x] `src/components/sessions/PromptHistoryDropdown.tsx`ファイルが作成されている
- [x] GET `/api/prompts`からプロンプト履歴を取得する
- [x] Headless UI Menuコンポーネントでドロップダウンを実装
- [x] 履歴選択時、onSelect(promptText)コールバックが呼ばれる
- [x] ローディング状態が表示される
- [x] エラー発生時、適切なメッセージが表示される
- [x] `src/components/sessions/__tests__/PromptHistoryDropdown.test.tsx`のすべてのテストが通過する
- [x] CreateSessionForm.tsxに統合されている
- [x] プロンプト入力欄の近くにドロップダウントリガーボタンが配置されている
- [x] ESLintエラーがゼロである
- [x] コミットが存在する
...
**受入基準**:
- [x] `src/components/git/CommitHistory.tsx`ファイルが作成されている
- [x] GET `/api/sessions/:id/commits`からコミット履歴を取得する
- [x] コミット一覧が時系列で表示される（ハッシュ、メッセージ、日時、変更ファイル数）
- [x] コミット選択時、diff表示エリアが更新される
- [x] 「このコミットに戻る」ボタンをクリックで確認ダイアログが表示される
- [x] ダイアログで確認後、POST `/api/sessions/:id/reset`が呼ばれる
- [x] リセット成功時、コミット履歴が再読み込みされる
- [x] `src/components/git/__tests__/CommitHistory.test.tsx`のすべてのテストが通過する
- [x] セッション詳細ページに「Commits」タブが追加されている
- [x] タブ切り替えでCommitHistoryコンポーネントが表示される
- [x] ESLintエラーがゼロである
- [x] コミットが存在する
...
**受入基準**:
- [x] SessionWebSocketHandlerがRunScriptManagerのlogイベントを転送する
- [x] `src/components/scripts/ScriptLogViewer.tsx`ファイルが作成されている
- [x] ログ一覧が時刻、ログレベル、メッセージを含めて表示される
- [x] ログレベルフィルター（info/warn/error）が機能する（REQ-035）
- [x] テキスト検索機能が実装されている（REQ-036）
- [x] ランスクリプト終了時、終了コードと実行時間が表示される（REQ-037）
- [x] 自動スクロールで最新ログが常に表示される
- [x] セッション詳細ページに「Scripts」タブが追加されている
- [x] ランスクリプト一覧と実行/停止ボタンが表示される
- [x] ランスクリプト実行中、ログがリアルタイムで表示される
- [x] `src/components/scripts/__tests__/ScriptLogViewer.test.tsx`が作成され、すべてのテストが通過する
- [x] ESLintエラーがゼロである
- [x] コミットが存在する
...
- [x] タスク24.1が完了している（GitStatusBadge実装）
- [x] タスク24.2が完了している（PromptHistoryDropdown実装）
- [x] タスク24.3が完了している（CommitHistory実装）
- [x] タスク24.4が完了している（ランスクリプトログUI実装）
- [x] すべてのテストが通過している（`npm test`）
- [x] ESLintエラーがゼロである
- [x] セッション一覧でGit状態インジケーターが表示される
- [x] 新規セッション作成時、プロンプト履歴ドロップダウンが機能する
- [x] セッション詳細ページ「Commits」タブでコミット履歴が閲覧できる
- [x] セッション詳細ページ「Scripts」タブでランスクリプトログが表示される
- [x] すべてのコミットが作成されている
...
**受入基準**:
- [x] MessageDisplay.tsxでsub_agentsフィールドを処理している
- [x] サブエージェント出力が折りたたみ可能なUIで表示される
- [x] 各サブエージェントタスクの名前、ステータス、出力が表示される
- [x] Headless UI Disclosureで展開/折りたたみが実装されている
- [x] デフォルトで折りたたまれている状態
- [x] 展開/折りたたみアニメーションがスムーズ
- [x] `src/components/sessions/__tests__/MessageDisplay.test.tsx`にテストが追加され、通過する
- [x] ESLintエラーがゼロである
- [x] コミットが存在する
...
**受入基準**:
- [x] ScriptLogViewerが`run_script_end`イベントを処理する
- [x] ランスクリプト終了時、終了コードが表示される
- [x] 終了コード0は緑、0以外は赤で表示される
- [x] 実行時間がhh:mm:ss形式で表示される
- [x] ログUI下部または最終行に終了情報エリアが配置されている
- [x] `src/components/scripts/__tests__/ScriptLogViewer.test.tsx`に終了情報表示テストが追加され、通過する
- [x] ESLintエラーがゼロである
- [x] コミットが存在する
...
- [x] タスク25.1が完了している（サブエージェント表示確認・実装）
- [x] タスク25.2が完了している（ランスクリプト終了コード表示確認）
- [x] サブエージェント出力が折りたたみ可能なUIで表示される
- [x] ランスクリプト終了時、終了コードと実行時間が表示される
- [x] すべてのテストが通過している（`npm test`）
- [x] ESLintエラーがゼロである
- [x] すべてのコミットが作成されている
...
|----------|----------|----------|----------|------------|
| 30.1 | playwright.config.ts環境変数修正 | なし | 10分 | `DONE` |
| 30.2 | e2eテストファイルのトークン参照修正 | 30.1 | 15分 | `DONE` |
...
**受入基準**:
- [x] `playwright.config.ts`で`AUTH_TOKEN`が`CLAUDE_WORK_TOKEN`に変更されている
- [x] ESLintエラーがゼロである
- [x] コミットが存在する
...
**受入基準**:
- [x] すべてのE2Eテストファイルで`AUTH_TOKEN`が`CLAUDE_WORK_TOKEN`に変更されている
- [x] E2Eテストが正常に動作する（`npm run e2e`）
- [x] ESLintエラーがゼロである
- [x] コミットが存在する
...
- [x] タスク30.1が完了している（playwright.config.ts修正）
- [x] タスク30.2が完了している（E2Eテストファイル修正）
- [x] E2Eテストが正常に実行できる
- [x] ESLintエラーがゼロである
- [x] すべてのコミットが作成されている
...
**受入基準**:
- [x] テストファイル`src/lib/__tests__/notification-service.test.ts`が存在する
- [x] テストが8つ以上含まれている
- [x] 実装前にテストのみのコミットが存在する
- [x] `src/lib/notification-service.ts`が実装されている
- [x] `getSettings()`がローカルストレージから設定を読み込む
- [x] `saveSettings()`が設定をローカルストレージに保存する
- [x] `requestPermission()`がNotification.requestPermission()を呼び出す
- [x] `sendNotification()`がタブ状態に応じてtoast/OS通知を送信する
- [x] すべてのテストが通過する（`npm test`）
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] テストファイル`src/store/__tests__/notification.test.ts`が存在する
- [x] テストが5つ以上含まれている
- [x] 実装前にテストのみのコミットが存在する
- [x] `src/store/notification.ts`が実装されている
- [x] `useNotificationStore`がエクスポートされている
- [x] `requestPermission`アクションが許可状態を更新する
- [x] `updateSettings`アクションが設定を更新しローカルストレージに保存する
- [x] すべてのテストが通過する（`npm test`）
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] `handleWebSocketMessage`が`status_change`で`completed`時に通知を送信する
- [x] `handleWebSocketMessage`が`status_change`で`error`時に通知を送信する
- [x] `handleWebSocketMessage`が`permission_request`時に通知を送信する
- [x] 既存のテストが通過する
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] セッションページ初回アクセス時に通知許可ダイアログが表示される
- [x] 許可/拒否後、再度ダイアログは表示されない
- [x] 許可状態がZustandストアに保存される
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] テストファイル`src/components/common/__tests__/NotificationSettings.test.tsx`が存在する
- [x] テストが5つ以上含まれている
- [x] 実装前にテストのみのコミットが存在する
- [x] `src/components/common/NotificationSettings.tsx`が実装されている
- [x] `src/components/layout/Header.tsx`にNotificationSettingsがインポート・配置されている
- [x] ヘッダーに通知アイコンが表示される（ThemeToggleの左隣）
- [x] クリックで設定ドロップダウンが開く
- [x] ドロップダウン外クリックで閉じる
- [x] チェックボックスで各イベントの通知をオン/オフできる
- [x] 設定変更が即座にローカルストレージに保存される
- [x] 通知がブロックされている場合に警告が表示される
- [x] すべてのテストが通過する（`npm test`）
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] OS通知クリック時にウィンドウがフォーカスされる
- [x] OS通知クリック時に該当セッションページに遷移する
- [x] 通知が自動的に閉じる
- [x] ESLintエラーがゼロである
...
- [x] タスク34.1が完了している（通知サービス）
- [x] タスク34.2が完了している（通知ストア）
- [x] タスク34.3が完了している（WebSocketハンドラ統合）
- [x] タスク34.4が完了している（許可リクエスト自動実行）
- [x] タスク34.5が完了している（設定UI）
- [x] タスク34.6が完了している（通知クリックフォーカス）
- [x] タスク完了時にOS通知が送信される
- [x] 権限要求時にOS通知が送信される
- [x] エラー発生時にOS通知が送信される
- [x] タブがアクティブな場合はtoast通知が表示される
- [x] 通知設定UIで各イベントのオン/オフが可能
- [x] 設定がローカルストレージに永続化される
- [x] すべてのテストが通過している（`npm test`）
- [x] ESLintエラーがゼロである
- [x] すべてのコミットが作成されている
...
**受入基準**:
- [x] Terminalタブをクリックしてもクラッシュしない
- [x] ターミナルが正常に表示される
- [x] コンソールにエラーが出力されない
- [x] ターミナルに入力できる（REQ-060）
- [x] ANSIエスケープシーケンスが正しくレンダリングされる（REQ-061）
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] セッション名/カードをクリックするとセッション詳細ページに遷移する
- [x] 遷移先URLが `/sessions/{sessionId}` である
- [x] クリック可能なことを示すカーソルスタイル（pointer）が適用されている
- [x] ホバー時に視覚的フィードバックがある
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] プロジェクトカードに「設定」ボタンがある
- [x] 「設定」ボタンクリックで設定モーダル/ページが開く
- [x] デフォルトモデルを選択できる（REQ-032）
- [x] ランスクリプトを追加・編集・削除できる
- [x] 保存後、セッションのScriptsタブにスクリプトが表示される
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] セッション一覧に各セッションのステータスアイコンが表示される
- [x] ステータスに応じたアイコンと色が適用される
- [x] initializingステータスではアイコンが回転する
- [x] アイコンにホバーするとステータス名がツールチップで表示される
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] Gitリポジトリでないパスを入力した場合、「Gitリポジトリではありません」と表示される
- [x] 存在しないパスの場合は別のエラーメッセージが表示される
...
- [x] タスク38.1が完了している（Terminalタブクラッシュ修正）
- [x] タスク38.2が完了している（セッション一覧からの遷移）
- [x] タスク38.3が完了している（プロジェクト設定UI）
- [x] タスク38.4が完了している（ステータスアイコン）
- [x] タスク38.5が完了している（エラーメッセージ）
- [x] すべてのテストが通過している（`npm test`）
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] Terminalタブをクリックするとターミナルヘッダーが表示される
- [x] 接続状態インジケーター（Connected/Disconnected）が表示される
- [x] XTerm.jsターミナルが描画される
- [x] ターミナルへの入力が可能である
- [x] WebSocket経由でPTYと通信できる
- [x] ウィンドウリサイズ時にターミナルがリサイズされる
- [x] ESLintエラーがゼロである
...
- [x] タスク39.1が完了している（ターミナル表示修正）
- [x] Terminalタブでターミナルが正常に表示される
- [x] すべてのテストが通過している（`npm test`）
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] `resume_session_id`カラムがsessionsテーブルに存在する
- [x] `last_activity_at`カラムがsessionsテーブルに存在する
- [x] Prismaクライアントが正しく生成される
- [x] 既存のセッションデータが保持される
...
**受入基準**:
- [x] ProcessLifecycleManagerクラスが実装されている
- [x] シングルトンパターン（globalThis対応）が実装されている
- [x] アクティビティトラッキング機能が実装されている
- [x] アイドルプロセス検出機能が実装されている
- [x] グレースフルシャットダウン機能が実装されている
- [x] すべてのテストが通過する（`npm test`）
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] StartOptionsに`resume`プロパティが追加されている
- [x] `claude --resume <session-id>`形式でプロセスが起動できる
- [x] resumeオプションなしの既存動作が維持される
- [x] すべてのテストが通過する（`npm test`）
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] SIGTERM受信時に全プロセスが停止される
- [x] SIGINT受信時に全プロセスが停止される
- [x] WebSocketクライアントにserver_shutdownメッセージが送信される
- [x] 5秒のグレース期間後にサーバーが終了する
- [x] ログに適切なシャットダウンメッセージが出力される
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] PROCESS_IDLE_TIMEOUT_MINUTES環境変数が読み取られる
- [x] 1分間隔でアイドルチェックが実行される
- [x] タイムアウト超過プロセスが停止される
- [x] セッションステータスが'paused'に更新される
- [x] WebSocketでprocess_pausedメッセージが送信される
- [x] resume_session_idがDBに保存される
- [x] タイムアウト0で機能が無効化される
- [x] すべてのテストが通過する（`npm test`）
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] POST /api/sessions/{id}/resumeエンドポイントが実装されている
- [x] paused状態のセッションが再開できる
- [x] resume_session_idがある場合--resumeで起動される
- [x] 再開後のステータスがrunningになる
- [x] paused以外の状態で400エラーが返される
- [x] 存在しないセッションで404エラーが返される
- [x] すべてのテストが通過する（`npm test`）
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] WebSocketメッセージ型にライフサイクルイベントが追加されている
- [x] process_pausedメッセージが送信できる
- [x] process_resumedメッセージが送信できる
- [x] server_shutdownメッセージが送信できる
- [x] すべてのテストが通過する（`npm test`）
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] paused状態のセッションで「一時停止中」が表示される
- [x] 再開ボタンが表示され、クリックでセッションが再開される
- [x] 再開後にチャット/ターミナルが利用可能になる
- [x] server_shutdown受信時にトースト通知が表示される
- [x] ESLintエラーがゼロである
...
**受入基準**:
- [x] ENV_VARS.mdに新しい環境変数が記載されている
- [x] 各環境変数のデフォルト値と説明がある
- [x] CLAUDE.mdの該当セクションが更新されている
...
**受入基準**:
- [x] E2Eテストが作成されている
- [x] アイドルタイムアウトのテストが通過する
- [x] セッション再開のテストが通過する
- [x] サーバーシャットダウン通知のテストが通過する
- [x] すべてのE2Eテストが通過する（`npm run e2e`）
...
- [x] タスク40.1が完了している（Prismaスキーマ更新）
- [x] タスク40.2が完了している（ProcessLifecycleManager実装）
- [x] タスク40.3が完了している（--resumeオプション追加）
- [x] タスク40.4が完了している（シャットダウンハンドラ）
- [x] タスク40.5が完了している（アイドルタイムアウトチェッカー）
- [x] タスク40.6が完了している（セッション再開API）
- [x] タスク40.7が完了している（WebSocketライフサイクルメッセージ）
- [x] タスク40.8が完了している（フロントエンド対応）
- [x] タスク40.9が完了している（ドキュメント更新）
- [x] タスク40.10が完了している（統合テスト）
- [x] すべてのテストが通過している（`npm test`）
- [x] すべてのE2Eテストが通過している（`npm run e2e`）
- [x] ESLintエラーがゼロである
