...
| フェーズ | 内容 | ステータス |
|---------|------|-----------|
| Phase 1-4 | MVP基盤構築 | `DONE` |
| Phase 5-7 | 拡張機能 | `DONE` |
| Phase 8-10 | バグ修正 | `DONE` |
| Phase 19-25 | UI/UX改善・機能修正 | `DONE` |
| Phase 30-41 | 各種不具合修正 | `DONE` |
| Phase 42 | セッション操作後のUI自動更新 | `DONE` |
| Phase 43 | UX改善機能（ターミナルリサイズ、Tree表示等） | `DONE` |
| Phase 45 | 認証機能の削除 | `DONE` |
| Phase 47 | Terminal WebSocketのDocker環境対応 | `DONE` |
...
### リスク5: DockerAdapterのシェル起動失敗

**影響度**: 中
**発生確率**: 低
**軽減策**:
- 手動でdocker runコマンドをテスト
- エラーメッセージの明確化
...
- 完了済みフェーズの詳細: `docs/archive/completed-phases.md`
- 検証レポート: `docs/verification-*.md`

### タスク管理のガイドライン

**AIエージェント向けタスクの粒度目安（作業時間）**:
...
**情報の明確性チェックの原則**:
1. ユーザーから「明示されている情報」と「不明な情報」を明確に分ける
2. 「おそらく〜だろう」という推測は「不明」として扱う
3. 不明な情報が1つでもあれば、実装前に必ず確認する
