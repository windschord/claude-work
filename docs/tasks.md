...
**受入基準**:
- [ ] `name`の更新が動作する
- [ ] `path`の更新が動作する（Git リポジトリ検証付き）
- [ ] `run_scripts`の追加・編集・削除が動作する
- [ ] APIテストが通る
...
**受入基準**:
- [ ] HOST環境のセッションに緑色の「H」バッジが表示される
- [ ] DOCKER環境のセッションに青色の「D」バッジが表示される
- [ ] SSH環境のセッションに紫色の「S」バッジが表示される
- [ ] バッジにホバーすると環境名がツールチップで表示される
- [ ] テストが追加されパスする
...
**受入基準**:
- [ ] プロジェクト設定モーダルで名前を変更すると保存される
- [ ] プロジェクト設定モーダルでパスを変更すると保存される
- [ ] プロジェクト設定モーダルでランスクリプトを追加・編集・削除すると保存される
- [ ] HOST環境のセッションに「H」バッジが表示される
- [ ] DOCKER環境のセッションに「D」バッジが表示される
- [ ] 全テストがパスする
