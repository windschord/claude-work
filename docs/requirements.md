...
- **REQ-005**: E2Eテストを実行した時、システムは `claude-work start` / `claude-work stop` の基本コマンドが動作することを検証しなければならない
- **REQ-006**: E2Eテストを実行した時、システムは `claude-work help` がヘルプメッセージを出力することを検証しなければならない
