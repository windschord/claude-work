...
**私は** 開発者として
**〜したい** 音声でClaude Codeに指示を送信
**なぜなら** キーボード入力が困難な状況でもハンズフリーで操作したいから

#### 受入基準（EARS記法）

- REQ-094: ユーザーが音声入力ボタンをクリックした時、システムはマイクの使用許可を要求しなければならない
- REQ-095: マイクが許可された時、システムは音声認識を開始し、録音中であることを視覚的に表示しなければならない
- REQ-096: 音声認識中、システムは中間結果（グレー表示）と確定結果（通常表示）を区別して入力フォームに表示しなければならない
- REQ-097: ユーザーが音声入力ボタンを再度クリックした時、または無音が2000ms（設定可能）続いた時、システムは音声認識を停止しなければならない
- REQ-098: 音声認識が完了した時、システムは確定結果のみを入力フォームに挿入しなければならない
- REQ-099: 自動送信が有効な場合、システムは音声認識完了時に自動的にメッセージを送信しなければならない
- REQ-100: システムは音声認識の言語（日本語: ja-JP、英語: en-US）を設定できなければならない
- REQ-101: 音声入力機能に対応していないブラウザでは、システムは音声入力ボタンを非表示またはグレーアウトしなければならない

### ストーリー18: 音声読み上げ

**私は** 開発者として
**〜したい** Claude Codeの出力を音声で読み上げ
**なぜなら** 画面を見ずに応答内容を確認したいから

#### 受入基準（EARS記法）

- REQ-102: 各アシスタントメッセージにおいて、システムは読み上げボタンを表示しなければならない
- REQ-103: ユーザーが読み上げボタンをクリックした時、システムはそのメッセージを音声で読み上げなければならない
- REQ-104: 読み上げ中、システムは再生中であることを視覚的に表示し、停止ボタンを提供しなければならない
- REQ-105: ユーザーが停止ボタンをクリックした時、システムは読み上げを即座に停止しなければならない
- REQ-106: 自動読み上げが有効な場合、システムは新しいアシスタントメッセージをキューに追加し、順番に読み上げなければならない
- REQ-107: システムは読み上げの音声、速度（0.5〜2.0倍速）、ピッチ（0〜2）を設定できなければならない
- REQ-108: コードブロックの読み上げはデフォルトでスキップし、設定で変更できなければならない
- REQ-109: 音声読み上げ機能に対応していないブラウザでは、システムは読み上げボタンを非表示またはグレーアウトしなければならない

### ストーリー19: 音声設定

**私は** 開発者として
**〜したい** 音声機能の設定を管理
**なぜなら** 自分の好みや環境に合わせてカスタマイズしたいから

#### 受入基準（EARS記法）

- REQ-110: システムは音声設定パネルを提供しなければならない
- REQ-111: 音声設定において、システムは音声入力の有効/無効を切り替えられなければならない
- REQ-112: 音声設定において、システムは音声読み上げの有効/無効を切り替えられなければならない
- REQ-113: 音声設定において、システムは認識言語を選択できなければならない
- REQ-114: 音声設定において、システムは読み上げ音声を利用可能な音声から選択できなければならない
- REQ-115: 音声設定において、システムは読み上げ速度（0.5〜2.0倍速）を調整できなければならない
- REQ-116: システムは音声設定をローカルストレージ（キー: `voice-settings`）に保存し、次回アクセス時に復元しなければならない。保存対象は設定値のみとし、一時的な状態（inputStatus等）は除外する
