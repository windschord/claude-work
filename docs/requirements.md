...
**私は** 開発者として
**〜したい** セッション画面から直接PRを作成し、PRページにアクセスしたい
**なぜなら** ブラウザやターミナルを切り替えずにワークフローを完結できるから

#### 受入基準（EARS記法）

- REQ-094: セッション詳細のDiffタブにおいて、システムは「PRを作成」ボタンを表示しなければならない
- REQ-095: ユーザーが「PRを作成」ボタンをクリックした時、システムはセッションのブランチ名とセッション名からghコマンドでPRを作成しなければならない
- REQ-096: PRが正常に作成された時、システムはPRのURLをクリック可能なリンク付きトースト通知で表示しなければならない
- REQ-097: ユーザーがトースト内のPRリンクをクリックした時、システムは新しいタブでPRページを開かなければならない
- REQ-098: PR作成が失敗した時、システムはエラーメッセージをトースト通知で表示しなければならない
