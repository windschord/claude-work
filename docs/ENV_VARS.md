...
- **備考**: Docker Compose環境ではコンテナ内に Claude CLI が含まれているため、通常は設定不要
...
- **形式**: 整数（GID）
- **例**: `DOCKER_GID=999`
- **デフォルト**: `0`（root グループ。Linux環境ではホストのdockerグループGIDに変更が必要）
- **取得方法**: `stat -c '%g' /var/run/docker.sock`
- **備考**: `docker-compose.yml` の `group_add` で使用される。macOS の Docker Desktop では設定不要（docker.sock のパーミッションが異なるため）

### GIT_REPOS_PATH

- **説明**: リモートリポジトリのクローン先パスを指定するための予約環境変数。現在の実装では使用されておらず、クローン先は `DATA_DIR` から `${DATA_DIR}/repos` として自動算出される
- **形式**: 絶対パス（将来の拡張用、現状は指定しても無視される）
- **例**: なし（`DATA_DIR` が `/data` の場合、クローン先は `/data/repos` 固定）
- **デフォルト**: `DATA_DIR` に基づき `${DATA_DIR}/repos` が使用される
...
HOST_PORT=3000
LOG_LEVEL=info
ALLOWED_ORIGINS=http://localhost:3000  # HOST_PORT を変更した場合はポート番号を合わせること
