...
```typescript
// Note: WebSocket型は 'ws' ライブラリの WebSocket を使用
// import type { WebSocket } from 'ws'

type EventHandler = (ws: WebSocket, eventName: string, payload: unknown) => void;
...
interface IConnectionManager {
  ...
  // ハンドラー管理
  registerHandler(sessionId: string, eventName: string, handler: EventHandler): void
