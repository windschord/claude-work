...
**受入基準**:
- [ ] Prisma パッケージが 7.x にアップグレードされている
- [ ] アダプターパッケージがインストールされている
...
**受入基準**:
- [ ] `prisma.config.ts` が作成されている
- [ ] `DATABASE_URL` が正しく参照されている
...
**受入基準**:
- [ ] provider が `prisma-client` になっている
- [ ] output が `./generated/prisma` に設定されている
...
**受入基準**:
- [ ] `prisma/generated/` が .gitignore に追加されている
...
**受入基準**:
- [ ] `prisma generate` がエラーなく完了する
- [ ] `prisma/generated/prisma/` にファイルが生成されている
...
**受入基準**:
- [ ] `PrismaBetterSqlite3` アダプターを使用している
- [ ] インポートパスが `prisma/generated/prisma` になっている
- [ ] シングルトンパターンが維持されている
...
**説明**:
- `@prisma/client` からのインポートを生成先パスに変更
...
**受入基準**:
- [ ] 全ての `@prisma/client` インポートが更新されている
