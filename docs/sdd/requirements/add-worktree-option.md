...
### 追加変更ファイル（branch_name空文字列ハンドリング）
- `src/app/api/sessions/[id]/route.ts` - worktree削除スキップ判定（branch_name空文字列で判定）
- `src/app/api/sessions/[id]/pr/route.ts` - PR作成時のbranch_name空文字列ガード
- `src/app/api/sessions/[id]/merge/route.ts` - マージ時のbranch_name空文字列ガード

### 変更しないファイル
- `src/db/schema.ts` - DBスキーマ変更不要（`claude_code_options`のJSON内で管理）
- `src/services/git-service.ts` - GitService自体は変更しない（branch_name空文字列ガードは呼び出し側のセッションAPIで制御）
