...
- **REQ-004**: もしスキーマ不一致がある場合、システムは修復方法を表示しなければならない（Docker Compose環境: `docker compose restart app` によるコンテナ再起動と自動マイグレーション。現在の実装では `npx drizzle-kit push` も案内）
...
To fix, restart the container: docker compose restart app
(auto-migration via docker-entrypoint.sh will re-sync the schema)
```
