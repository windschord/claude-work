...
**THE SYSTEM SHALL** Docker volume内のファイルを `docker run --rm -v <volume>:/workspace alpine find` で検索する。
...
**IF** プロジェクトの `clone_location` が `'docker'` の場合、
**THE SYSTEM SHALL** Docker volume内のファイルを `docker run --rm -v <volume>:/workspace alpine cat` で読み込む。
...
**THE SYSTEM SHALL** `fs.readdir` で再帰的にディレクトリを探索して `.env*` ファイルを検索し、除外ディレクトリをスキップする。
