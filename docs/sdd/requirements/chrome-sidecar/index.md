...
| REQ-001-003 | .mcp.jsonのargs配列に--browserUrlを注入 | US-001 | 定義済 |
| REQ-001-004 | Chrome Ready Check（最大30秒）後にClaude起動 | US-001 | 定義済 |
| REQ-002-001 | セッション破棄時にChromeコンテナ・ネットワークを削除 | US-002 | 定義済 |
| REQ-002-002 | 孤立Chromeコンテナの自動クリーンアップ（ラベルベース識別） | US-002 | 定義済 |
...
| REQ-004-001 | Chromeデバッグポートを127.0.0.1に動的マッピング | US-004 | 定義済 |
| REQ-004-002 | デバッグポート番号をUIで表示 | US-004 | 定義済 |
| REQ-004-002a | デバッグポート表示のアクセス制御（所有者/管理者限定） | US-004 | 定義済 |
