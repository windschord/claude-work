...
- REQ-004: `docker compose up -d` の実行で、ClaudeWorkが起動し http://localhost:3000 でアクセス可能にならなければならない
...
- NFR-002: ドキュメントの更新において、既存のnpx/systemdデプロイ方法の手順が失われてはならない（セカンドオプションとして残す）
