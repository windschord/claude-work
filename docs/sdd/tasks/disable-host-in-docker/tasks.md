...
**受入基準**:
- [x] 7つ以上のテストケースが作成されている
- [x] `npx vitest run src/lib/__tests__/environment-detect.test.ts` でテストが失敗する（実装なし）
- [x] テストがコミットされている
...
**受入基準**:
- [x] `src/lib/environment-detect.ts` が存在する
- [x] `isRunningInDocker()` が正しく動作する
- [x] `isHostEnvironmentAllowed()` が正しく動作する
- [x] `initializeEnvironmentDetection()` が結果をキャッシュする
- [x] タスク1.1のテストが全て通過する
...
**受入基準**:
- [x] `create()` がHOST不許可時にエラーをスロー
- [x] `checkStatus()` がHOST不許可時にavailable=falseを返す
- [x] `ensureDefaultExists()` がHOST不許可時にDocker環境をデフォルトに設定
- [x] 既存テストが全て通過する
- [x] `npm run lint` でエラーなし
...
**受入基準**:
- [x] GETレスポンスに `meta.hostEnvironmentDisabled` が含まれる
- [x] HOST環境に `disabled: true` が付与される（不許可時）
- [x] POST HOST作成で403が返る（不許可時）
- [x] 既存テストが全て通過する
- [x] `npm run lint` でエラーなし
...
**受入基準**:
- [x] HOST環境不許可時にセッション作成が403を返す
- [x] DOCKER環境のセッション作成は影響なし
- [x] 既存テストが全て通過する
...
**受入基準**:
- [x] `disabled` フィールドが型定義に含まれる
- [x] `hostEnvironmentDisabled` がフックから返される
- [x] 既存テストが通過する
...
**受入基準**:
- [x] HOST無効時にHOSTタイプが選択不可
- [x] HOST無効時にデフォルト選択がDOCKER
- [x] HOST無効時に理由が表示される
- [x] HOST有効時は既存動作と同じ
...
**受入基準**:
- [x] HOST環境カードが無効時にグレーアウト表示
- [x] 「無効」バッジが表示される
- [x] 無効時にステータスが「利用不可」表示
- [x] 通常時は既存動作と同じ
...
**受入基準**:
- [x] サーバー起動時に環境検出が実行される
- [x] 検出結果がログに出力される
- [x] `docker-compose.yml` に `RUNNING_IN_DOCKER=true` が含まれる
- [x] `ALLOW_HOST_ENVIRONMENT` がコメントアウトされた状態で記載
- [x] 全テストが通過する
...
**受入基準**:
- [x] `RUNNING_IN_DOCKER` の説明が記載されている
- [x] `ALLOW_HOST_ENVIRONMENT` の説明が記載されている
- [x] 使用例が記載されている
