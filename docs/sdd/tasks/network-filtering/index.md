...
> **iptables方式廃止**: 上表は旧iptables方式の実装計画に対する完了状況です。現在の機能状態としては、TASK-002（IptablesManager実装）およびTASK-005・TASK-006・TASK-012の一部（フィルタ適用・クリーンアップロジック）は削除済みで、REQ-004/005/007とNFR-SECの再充足はproxy方式（US-007）で別途対応予定です。
...
| ~~TASK-002~~ | ~~src/services/iptables-manager.ts~~ (削除済み) | なし |

### グループB（TASK-001完了後に実行可能）
...
### グループD（TASK-004完了後に並列実行可能）

| タスク | 対象ファイル | 依存 |
|--------|-------------|------|
| ~~TASK-005~~ | ~~src/services/network-filter-service.ts (適用部分)~~ (applyFilter等削除済み) | ~~TASK-002~~, TASK-004 |
...
| タスク | 対象ファイル | 依存 |
|--------|-------------|------|
| ~~TASK-006~~ | src/services/adapters/docker-adapter.ts (フィルタ統合ロジック削除済み) | ~~TASK-005~~ |
...
| TASK-012 | テストファイル群 (iptables関連テスト削除済み) | ~~TASK-006~~, TASK-009 |
...
- [-] REQ-007（モニタリング）: TASK-008, TASK-010 - 適用中ルールの表示はUS-007/proxy方式で再実装予定
