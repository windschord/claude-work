...
- [x] テンプレート適用ダイアログが正しく表示される
- [x] テンプレートのカテゴリ別表示が動作する
- [x] 全選択/全解除が動作する
- [x] テンプレート適用後に結果（created/skipped）が表示される
- [x] 通信テストダイアログが正しく表示される
- [x] テスト結果が色分けで表示される（許可=緑、ブロック=赤）
- [x] テストが13件以上あり、全て通過する
...
## 完了サマリー
TDDで実装完了。NetworkTemplateDialogとNetworkTestDialogを新規作成し、13件のテストが全てパス。
- NetworkTemplateDialog: テンプレート取得・カテゴリ別表示・全選択/全解除・適用結果表示
- NetworkTestDialog: 通信テスト・許可(緑)/ブロック(赤)の結果表示・マッチルール情報表示
