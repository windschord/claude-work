...
- [x] `resolveDomains` メソッドが実装されている
- [x] 通常ドメイン、ワイルドカード、IP、CIDRの全パターンが正しく処理される
- [x] DNSキャッシュが実装されている（TTL: 5分）
- [x] DNS解決失敗時にスキップ（フェイルオープンではなく警告）される
- [x] `testConnection` メソッドが実装されている（dry-run）
- [x] テストが15件以上あり、全て通過する
