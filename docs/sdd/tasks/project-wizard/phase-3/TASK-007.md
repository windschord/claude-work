...
2. テスト実行: 失敗を確認
3. テストコミット
4. WizardContainer.txtに各ステップを統合実装
...
- [ ] WizardContainerに全4ステップが統合されている
- [ ] ProjectList.tsx、Sidebar.tsxのimportが置き換わっている
- [ ] 旧AddProjectModal関連ファイルが削除されている
- [ ] 旧RemoteRepoFormが削除されている
- [ ] テストファイルが存在し、全統合テストが通過
- [ ] `npx vitest run` で既存テストを含む全テストが通過
