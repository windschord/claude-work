...
1. `next.config.js` にrewrite設定を追加
...
- [x] `/api/registry-firewall/ui/` がregistry-firewallの管理UIにプロキシされる
- [x] 環境設定ページに「パッケージセキュリティ」セクションが表示される
- [x] ステータスが3色(green/red/gray)で表示される
- [x] トグルスイッチで有効/無効を切り替えられる
- [x] 「管理画面を開く」リンクが機能する
- [x] ブロックログが表示される
