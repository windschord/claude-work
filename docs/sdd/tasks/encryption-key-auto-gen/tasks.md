...
2. テストケース:
   ...
   - キーファイル書き込み時に `fs.writeFileSync` に `mode: 0o600` が指定されていること
3. テスト実行して失敗を確認
...
**推定工数**: 5分
**ステータス**: DONE
**完了サマリー**: TypeScript型チェックパス。全テストスイートパス。Docker環境でのPAT登録API動作検証: ENCRYPTION_KEY未設定のDocker環境(docker compose up)でcurlによるPOST /api/github-pat実行、修正前は500エラー(PATEncryptionError)、修正後はサーバー起動時にキー自動生成されPAT登録が正常動作(キーファイルdata/encryption.keyが自動作成)。AC-004に合致。
**要件対応**: AC-004
...
|---------|---------|---------|---------|------|----------|
| 1.1 | テスト作成 | 1 | 10分 | - | DONE |
| 1.2 | ensureEncryptionKey実装 | 1 | 10分 | 1.1 | DONE |
| 2.1 | server.ts統合 | 2 | 5分 | 1.2 | DONE |
| 2.2 | 動作確認 | 2 | 5分 | 2.1 | DONE |
