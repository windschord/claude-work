...
アプリ全体で単一のToasterコンポーネントを使用する。

**配置場所**: `src/app/layout.tsx`（ルートレイアウト）
**表示位置**: `top-right`（右上）

### 理由

`react-hot-toast` はシングルトンストアを使用しているため、複数の `<Toaster />` を同時にマウントすると以下の問題が発生する:

1. 同一内容の通知が複数箇所に表示される
2. 自動消去タイマーが競合して正常動作しない

### 実装ルール

- `<Toaster position="top-right" />` はルートレイアウト（`layout.tsx`）にのみ配置する
- 個別ページに `<Toaster />` を追加してはならない
- toast通知の表示位置を変更する場合は、ルートレイアウトのみを修正する
