...
[PortMappingList] --fetch--> [POST /api/environments/check-ports] --call--> [PortChecker]
                                                              ...
                                                              +-------------------+-------------------+
                                                              |                   |                   |
                                                    [checkHostPort]    [checkClaudeWorkPorts]  [checkContainerPort]
                                                    net.createServer()  DB query              docker exec
```
...
4. PortCheckerが以下を並行実行:
   - HOST側ポートバインド試行（`net.createServer()`）
   - ClaudeWork内他環境のポート設定との照合（DB検索）
   - コンテナ内ポートチェック（`docker exec`、コンテナ実行中の場合のみ）
5. 結果を集約してレスポンス返却
...
| REQ-002-001 | checkContainerPort() | PortChecker |
| REQ-002-002 | コンテナ未起動時スキップ | PortChecker |
| REQ-002-003 | docker exec失敗時unknown | PortChecker |
