...
ClaudeWorkは、Next.js統合アーキテクチャを採用する。フロントエンド（Pages/Components）、バックエンド（API Routes）、WebSocketサーバー（カスタムサーバー）を1つのNext.jsプロジェクトに統合し、`docker compose up -d`コマンドで起動する。
...
**根拠**:
- `docker compose up -d` 1コマンドで起動できるシンプルさ
