...
起動時のマイグレーションエラー（`SqliteError: no such column`）の根本原因は、スキーマ定義が2箇所（Drizzle Kit と CLI手動マイグレーション）に分散し、同期されていないことでした。

> **注**: 本ドキュメント作成時はSystemdがデプロイ方法でしたが、現在はDocker Composeが唯一のデプロイ方法です。`docker-entrypoint.sh`でマイグレーションが自動実行されます。
