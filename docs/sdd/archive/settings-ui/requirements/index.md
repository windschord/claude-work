...
### ストーリー7: 設定ページへの戻りナビゲーション

**私は** ユーザーとして
**〜したい** 各設定詳細ページから設定ページに簡単に戻りたい
**なぜなら** 他の設定カテゴリを確認したい時に、ブラウザの戻るボタンではなく明示的なUIで戻りたいから

#### 受入基準（EARS記法）

- **REQ-024**: 各設定詳細ページ（`/settings/app`、`/settings/environments`、`/settings/github-pat`）には、システムは戻るボタンを表示しなければならない
- **REQ-025**: ユーザーが戻るボタンをクリックした時、システムは `/settings` ページに遷移しなければならない
- **REQ-026**: 戻るボタンは視覚的に識別しやすい位置（ページ上部）に配置されなければならない
- **REQ-027**: 戻るボタンには適切なラベル（「設定に戻る」または矢印アイコン+「設定」）が表示されなければならない

### ストーリー8: 未保存変更の警告

**私は** ユーザーとして
**〜したい** アプリ設定で変更を保存せずに戻ろうとした時に警告を受け取りたい
**なぜなら** 意図せずに変更を失いたくないから

#### 受入基準（EARS記法）

- **REQ-028**: アプリ設定ページでフォームの値が変更された時、システムは変更を検出しなければならない
- **REQ-029**: ユーザーが未保存の変更がある状態で戻るボタンをクリックした時、システムは警告ダイアログを表示しなければならない
- **REQ-030**: 警告ダイアログは「変更が保存されていません。破棄しますか？」というメッセージを含まなければならない
- **REQ-031**: 警告ダイアログには「破棄して戻る」と「キャンセル」の2つのボタンを表示しなければならない
- **REQ-032**: ユーザーが「破棄して戻る」を選択した時、システムは変更を破棄して `/settings` ページに遷移しなければならない
- **REQ-033**: ユーザーが「キャンセル」を選択した時、システムはダイアログを閉じて現在のページに留まらなければならない
- **REQ-034**: フォームを保存した後は、システムは変更済みフラグをリセットしなければならない
...
- **NFR-017**: 未保存変更の警告ダイアログは100ms以内に表示されなければならない
...
- **NFR-015**: 戻るボタンは全ての設定詳細ページで同じデザインと配置であること
- **NFR-016**: 未保存変更の警告ダイアログは明確で理解しやすい文言であること
...
- **NFR-018**: 戻るボタンはキーボードナビゲーションとスクリーンリーダーに対応していること
- **NFR-019**: 未保存変更の警告ダイアログはキーボード操作（ESCキーでキャンセル、Enterキーで確定）に対応していること
...
5. **ナビゲーションの改善**: 全ての設定詳細ページから設定ページへ1クリックで戻れる
6. **データ損失防止**: 未保存の変更がある場合に警告が表示され、意図しないデータ損失を防止
