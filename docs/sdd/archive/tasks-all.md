...
## Phase 49: 左メニューUI改善バグ修正

**推定期間**: 2時間（AIエージェント作業時間）

**関連要件**: REQ-188, REQ-198〜REQ-200

### 概要

Phase 48で実装した左メニューUI改善における以下のバグを修正:
1. プロジェクト設定変更APIが`name`のみ更新し、`path`と`run_scripts`を無視している
2. セッションの動作環境（HOST/DOCKER/SSH）がサイドバーに表示されない

### フェーズ1: API修正

#### タスク49.1: PUT /api/projects/[id] APIの修正

**説明**:
- `path`と`run_scripts`の更新をサポートするようAPIを修正
- RunScriptテーブルとのトランザクション処理を実装

**技術的文脈**:
- `src/app/api/projects/[project_id]/route.ts`を修正
- Prismaトランザクションで`RunScript`を一括更新
- パスの存在確認とGitリポジトリ検証を追加

**情報の明確性**:

| 分類 | 内容 |
|------|------|
| 明示された情報 | name, path, run_scriptsの更新対応 |
| 不明/要確認の情報 | なし |

**受入基準**:
- [x] `name`の更新が動作する
- [x] `path`の更新が動作する（Git リポジトリ検証付き）
- [x] `run_scripts`の追加・編集・削除が動作する
- [x] APIテストが通る

**TDD手順**:
1. 既存のAPIテストを確認し、path/run_scripts更新のテストケースを追加
2. テストを実行し失敗を確認
3. API実装を修正
4. テストをパスさせる
...
#### タスク49.2: SessionTreeItemに動作環境バッジを追加

**説明**:
- セッションの動作環境タイプ（HOST/DOCKER/SSH）をバッジで表示
- 環境タイプに応じた色分け（HOST=緑、DOCKER=青、SSH=紫）
- ホバー時に環境名をツールチップで表示

**技術的文脈**:
- `src/components/layout/SessionTreeItem.tsx`を修正
- Session型に`environment_type`と`environment_name`が既に存在
- 短縮表示（H/D/S）でコンパクトに表示

**情報の明確性**:

| 分類 | 内容 |
|------|------|
| 明示された情報 | 環境タイプバッジ、色分け、ツールチップ |
| 不明/要確認の情報 | なし |

**受入基準**:
- [x] HOST環境のセッションに緑色の「H」バッジが表示される
- [x] DOCKER環境のセッションに青色の「D」バッジが表示される
- [x] SSH環境のセッションに紫色の「S」バッジが表示される
- [x] バッジにホバーすると環境名がツールチップで表示される
- [x] テストが追加されパスする

**TDD手順**:
1. SessionTreeItemテストに環境バッジのテストケースを追加
2. テストを実行し失敗を確認
3. コンポーネントを修正
4. テストをパスさせる
...
**説明**:
- 修正した機能のブラウザでの動作確認
- プロジェクト設定変更が正しく保存されることを確認
- セッションの動作環境バッジが表示されることを確認

**技術的文脈**:
- 本番モードで動作確認
- 複数の環境タイプ（HOST/DOCKER）でテスト

**情報の明確性**:

| 分類 | 内容 |
|------|------|
| 明示された情報 | 設定変更と環境バッジの動作確認 |
| 不明/要確認の情報 | なし |

**受入基準**:
- [x] プロジェクト設定モーダルで名前を変更すると保存される
- [x] プロジェクト設定モーダルでパスを変更すると保存される
- [x] プロジェクト設定モーダルでランスクリプトを追加・編集・削除すると保存される
- [x] HOST環境のセッションに「H」バッジが表示される
- [x] DOCKER環境のセッションに「D」バッジが表示される
- [x] 全テストがパスする

**依存関係**: タスク49.1, タスク49.2
**推定工数**: 15分
**ステータス**: `DONE`
