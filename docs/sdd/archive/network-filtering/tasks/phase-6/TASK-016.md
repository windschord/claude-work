...
- [x] フィルタリング有効時にNetworkModeが設定される
- [x] フィルタリング有効時にHTTP_PROXY/HTTPS_PROXYが設定される
- [x] フィルタリング無効時は現行動作を維持
- [x] コンテナ起動後にルール同期が行われる
- [x] コンテナ停止時にルールクリーンアップが行われる
- [x] proxyヘルスチェック失敗時はフェイルセーフ（起動中止）
- [x] テストが7つ以上追加されている
- [x] 既存テストが壊れていない
- [x] ESLintエラーがゼロ
