...
- **REQ-131**: セッションがワンクリック作成された時、システムはグローバル設定のデフォルトモデルを使用しなければならない。デフォルトモデルが未設定の場合は、モデル選択ダイアログを表示するか、定義済みのフォールバックモデル（claude-sonnet-4-20250514）を使用する
...
- **REQ-147**: ユーザーがプロジェクトノードをクリックして折りたたんだ時、システムはその状態をローカルストレージ（キー: `claudework:tree-collapse-state`）に保存しなければならない
...
- **NFR-028**: Tree表示の展開/折りたたみ状態はブラウザセッション間で永続化されなければならない（ブラウザを閉じて再度開いた後も維持される）
