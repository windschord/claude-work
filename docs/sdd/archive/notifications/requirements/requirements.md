...
- **REQ-089**: ブラウザタブがバックグラウンドの場合、システムはOS通知を送信しなければならない。ただし、通知権限が未許可（default または denied）の場合は、toast通知にフォールバックする
...
- **NFR-001**: システムはClaude Codeの出力を500ms以内にブラウザに表示しなければならない。測定起点はサーバー側WebSocketでのデータ受信時刻、終点はブラウザ側でのDOM更新完了時刻とする
