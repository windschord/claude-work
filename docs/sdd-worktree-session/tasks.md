...
- ローカルリポジトリ登録時: FilesystemServiceでGit判定、デフォルトブランチ取得
  - `git symbolic-ref --short HEAD` を使用（空リポジトリ対応）
  - コミットがない場合でもブランチ名を取得可能
...
- セッション名はモーダル表示時に自動生成（session-name-generator.ts）
- 再生成ボタン（RefreshCwアイコン）で新しい名前を生成
