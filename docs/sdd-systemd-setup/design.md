...
**背景**:
`.npmignore` が存在しない場合、npm は `.gitignore` をパッケージのフィルタリングに使用する。`.gitignore` には `.next/` と `dist/` が含まれているため、npx 経由でインストールした際にビルド成果物が除外され、Next.js のモジュール解決が失敗する問題があった（PR #71 で修正）。
