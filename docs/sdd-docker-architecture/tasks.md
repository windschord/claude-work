...
#### タスク8.1: ルートページのリダイレクト

**説明**:
- 対象ファイル: `src/app/page.tsx`
- ルートページ (`/`) を `/docker` にリダイレクト
- 旧ProjectListの使用を停止

**技術的文脈**:
- Next.js の redirect() 関数を使用
- または `/docker/page.tsx` の内容で置き換え

**受入基準**:
- [x] `/` にアクセスすると `/docker` と同じUIが表示される
- [x] ProjectList コンポーネントが使用されていない
- [x] ビルドが成功する
...
#### タスク8.2: 旧プロジェクトページの削除

**説明**:
削除対象:
- `src/app/projects/` ディレクトリ全体
- `src/app/sessions/` ディレクトリ全体（旧セッションページ）

**技術的文脈**:
- 旧アーキテクチャのページは不要
- Docker セッションは `/docker` で管理

**受入基準**:
- [x] `src/app/projects/` が削除されている
- [x] `src/app/sessions/` が削除されている
- [x] ビルドが成功する
...
#### タスク8.3: 旧プロジェクトコンポーネントの削除

**説明**:
削除対象:
- `src/components/projects/` ディレクトリ全体
- `src/components/layout/MainLayout.tsx`（fetchProjects使用）
- `src/components/layout/Sidebar.tsx`（プロジェクト一覧表示）

**技術的文脈**:
- Docker セッションUIは独自レイアウトを使用
- MainLayout/Sidebar は旧アーキテクチャ専用

**受入基準**:
- [x] `src/components/projects/` が削除されている
- [x] `src/components/layout/MainLayout.tsx` が削除されている
- [x] `src/components/layout/Sidebar.tsx` が削除されている
- [x] ビルドが成功する
...
#### タスク8.4: ストア・フックのクリーンアップ

**説明**:
- 対象ファイル: `src/store/index.ts` 他
- プロジェクト関連の状態管理を削除
- 不要なフックを削除

**技術的文脈**:
- fetchProjects, projects などの状態を削除
- Docker セッション用の状態は useDockerSessions フックで管理

**受入基準**:
- [x] プロジェクト関連の状態が削除されている
- [x] 不要なフックが削除されている
- [x] ビルドが成功する
- [x] テストが通過する
...
#### タスク8.5: ヘッダー・ナビゲーションの更新

**説明**:
- 対象ファイル: `src/components/layout/Header.tsx`
- ナビゲーションをDockerセッション中心に更新
- 不要なリンクを削除
