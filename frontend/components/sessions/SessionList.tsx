    ...
    <div className="space-y-3 md:space-y-2">
      {sessions.map((session) => (
        <div
          ...
          className="bg-white border border-gray-200 rounded-lg p-4 md:p-4 hover:shadow-md transition-shadow cursor-pointer active:bg-gray-50"
        >
          <div className="flex items-start justify-between">
            <div className="flex-1 min-w-0">
              <h3 className="text-base md:text-sm font-semibold text-gray-900 truncate">
                {session.name}
              </h3>
              <p className="text-sm md:text-xs text-gray-500 mt-2 md:mt-1">
                ブランチ: {session.branch_name}
              </p>
              <p className="text-sm md:text-xs text-gray-400 mt-1 md:mt-0.5">
